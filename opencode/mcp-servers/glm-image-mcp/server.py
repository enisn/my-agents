...
def validate_size(size: str, model: str) -> bool:
    ...
    # Check if it's a recommended size
    ...
    try:
        ...
        # Custom size validation based on model requirements
...
def generate_image(
    ...
    try:
        ...
        # Validate and set quality if model supports it
...
@mcp.tool()
async def list_supported_models() -> str:
    """
...
@mcp.tool()
async def list_supported_sizes(model: str = None) -> str:
    """
...
@mcp.tool()
async def list_supported_qualities() -> str:
    """
...
@mcp.tool()
async def validate_image_params(
...
if __name__ == "__main__":
    ...
    if not ZAI_CREDENTIAL:
        print("Warning: ZAI_API_KEY environment variable not set.", file=sys.stderr)
        print("The server will start but will fail on API calls.", file=sys.stderr)
        print()
